...
def initialize_model(model_choice, task, cv_folds, parameter_grid, gs_metric, seed, mlp_solver = 'adam'):
    ...
    if model_choice == "linear":
        if task == 'binary':
            ...
            est = LogisticRegression(random_state=seed, solver = 'saga', max_iter = 10000)
        elif task == 'multiclass':
            from sklearn.linear_model import LogisticRegression
            param_grid = parameter_grid if parameter_grid is not None else {'C': np.arange(0.001, 1, 0.05), 'penalty': ['l2','l1'], 'max_iter': [10000]}
            est = LogisticRegression(random_state=seed, multi_class = 'multinomial', solver = 'saga', max_iter = 10000)
        elif task == 'continuous':
        ...
        if task in ['binary','multiclass']:
            ...
            est = LinearSVC(max_iter = 100000, dual=False, penalty = 'l2')
        elif task == 'continuous':
            from sklearn.svm import LinearSVR
            est = LinearSVR(max_iter = 100000, dual=False, loss = 'squared_epsilon_insensitive')
        gs = GridSearchCV(estimator = est, param_grid=param_grid, scoring=gs_metric, cv= cv_folds)
